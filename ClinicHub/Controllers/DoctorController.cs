...
namespace ClinicHub.Controllers
{
    public class DoctorController : BaseController
    {
        ...
        public DoctorController(IDoctorService doctorService, IDoctorDashboardService doctorDashboardService, IAuthService authService, IAttachmentService attachmentService)
        {
        ...
        public async Task<IActionResult> Availability()
        {
            ...
            try
            {
                ...
                ViewBag.TypicalDuration = items
                    .GroupBy(i => i.SlotDurationMinutes)
                    .OrderByDescending(g => g.Count())
                    .FirstOrDefault()?.Key ?? 30;
            }
            catch (ApiException ex)
            {
                ViewBag.ErrorMessage = ex.Message;
        ...
        private static List<MockStat> BuildAvailabilityStats(List<DoctorAvailabilityDto> items)
        {
            ...
            return new()
            {
                ...
                new() { Value = $"{typicalDuration} دقيقة", Label = "مدة الحجز النموذجية", IconColor = "amber", SvgPath = "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z" },
            };
        }
    }
}
